...
export class ListPage {
  ...
  getSubsItem(subject:any,type:any){
    ...
    switch(type){
      ...
      case 3:moduleType=3;break;
      case 4:moduleType=4;break;
    ...
    if(tryOut||(this.subject.exam&&this.subject.time>=new Date().getTime())){
      if(this.type==1||this.type==2){
        ...
        this.httpstorage.getStorage("s"+this.subject.id+"s",(data)=>{
          ...
          this.navCtrl.push(ExamPage,{subject:this.subject,title:this.title,exams:exam,mode:false,time:totalTime});
        });
      }
    }
    else{
